...
import { Box, Button, styled, Table, TableBody, TableCell, TableContainer, TableRow, Typography } from '@mui/material';
...
const SearchResultList = ({ list, hasNextPage, isFetchingNextPage, fetchNextPage }: SearchResultListProps) => {
    ...
    const uniqueTracks = Array.from(new Map(list.map((track) => [track.id, track])).values());
    ...
    return (
        <TableContainer>
            {' '}
            {/* TableContainer를 추가하여 스크롤 및 스타일링 개선 */}
            <Table>
                <TableBody>
                    {uniqueTracks.map((track) => (
                        <StyledTableRow key={track.id}>
                            ...
                            <TableCell>
                                {/* onClick 핸들러에서 `track.uri`를 인자로 전달합니다. */}
                                <Button
                                    onClick={() => {
                                        if (track.uri) {
                                            // track.uri가 존재하는지 확인
                                            ...
                                            console.warn('Track URI is undefined, cannot add to playlist:', track);
